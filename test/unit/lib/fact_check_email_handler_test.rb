...
class FactCheckEmailHandlerTest < ActiveSupport::TestCase
  ...
  def build_gmail_message_with_subject(subject, mail)
    build_gmail_message(
      {
        Subject: subject,
      },
  ...
  test "#process ignores 'out of office' emails" do
    ...
    message = build_gmail_message_with_subject(subject, mail)
  ...
  test "#process sends count of unprocessed emails to Prometheus" do
    ...
    out_of_office_message = build_gmail_message_with_subject("Automatic reply: out of office", out_of_office_mail)
    other_mail = Mail.new { subject("Any other subject") }
    other_message = build_gmail_message_with_subject("Any other subject", other_mail)
