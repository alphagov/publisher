...
class SidekiqGdsSsoMiddlewareTest < ActiveSupport::TestCase
  setup do
    @user = FactoryBot.create(:user, :govuk_editor, name: "Some Person")
    @warden = mock("warden")
    @warden.stubs({ authenticated?: true, user: @user })
  end

  test "it will proxy a request to sidekiq web for an authenticated user with a 'sidekiq_adminn' permission" do
    @user.permissions = %w[sidekiq_admin]
    env = { "warden" => @warden }
  ...
  test "it will return a 403 forbidden response for an authenticated user without 'sidekiq_admin' permission" do
    @user.permissions = []
