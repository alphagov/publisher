...
class JsonImporterTaskTest < ActiveSupport::TestCase
  setup do
  ...
  context "LocalTransactionEdition" do
    should "inserts the record and an empty DevolvedAdministrationAvailability record for each nation when no devolved admin availability" do
      DevolvedAdministrationAvailability.count
      ...
      devolved_admin_records = DevolvedAdministrationAvailability.last(3)
      nations = %w[ScotlandAvailability WalesAvailability NorthernIrelandAvailability]
      assert_includes nations, devolved_admin_records[0].type
      assert_includes nations, devolved_admin_records[1].type
      assert_includes nations, devolved_admin_records[2].type

      devolved_admin_records = DevolvedAdministrationAvailability.last(3)
      devolved_admin_records.each do |record|
        assert_equal nil, record.alternative_url
      end
    end

    should "insert the record and copies the DevolvedAdministrationAvailability data from the json" do
      ...
      assert_equal "ScotlandAvailability", devolved_admin_records[0].type
      assert_equal "https://www.mygov.scot/self-isolation-grant",
                   devolved_admin_records[0].alternative_url
      assert_equal "WalesAvailability", devolved_admin_records[1].type
      assert_equal "https://gov.wales/self-isolation-support-scheme",
                   devolved_admin_records[1].alternative_url
      assert_equal "NorthernIrelandAvailability", devolved_admin_records[2].type
      assert_equal "https://www.nidirect.gov.uk/information-and-services/coronavirus-covid-19/financial-help-and-practical-support",
                   devolved_admin_records[2].alternative_url
    end
  end

  context "SimpleSmartAnswerEdition" do
    should "insert data correctly from json file record" do
  ...
  context "TransactionEdition" do
    ...
    should "insert data and variant correctly from json file record" do
  ...
  context "Users" do
    should "insert data correctly from json file record" do
      ...
      assert_equal %w[signin welsh_editor], User.last.permissions
  ...
  context "LocalService" do
    should "insert data correctly from json file record" do
      ...
      assert_equal %w[county unitary], LocalService.where(mongo_id: "4f340dce1d41c87e59000009").first.providing_tier
  ...
  context "LinkCheckReports" do
    should "insert LinkCheckReport and child Links correctly from json record" do
      ...
      assert_equal 24_084_930, link_check_report.batch_id
  ...
  context "Homepage PopularLinks" do
    should "insert PopularLinksEdition and child LinkItems correctly from json record" do
      ...
      popular_links_edition = PopularLinksEdition.last
      assert_equal 6, popular_links_edition.link_items.count
      assert_equal "/log-in-register-hmrc-online-services", popular_links_edition.link_items[0]["url"]
      assert_equal "HMRC account: sign in or set up", popular_links_edition.link_items[0]["title"]
    end
  end
end
