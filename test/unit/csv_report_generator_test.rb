...
class CsvReportGeneratorTest < ActiveSupport::TestCase
  ...
  test "editions_active_in_past_two_years returns editions created with past two years in desc order" do
    ...
    edition1 = FactoryBot.create(:transaction_edition,
                                 title: "Bringing your pet dog, cat or ferret to Great Britain", slug: "bring-pet-to-great-britain",
                                 created_at: Time.zone.now - 1.year, assigned_to_id: user.id)
    edition2 = FactoryBot.create(:transaction_edition,
                                 title: "Register to vote (armed forces)", slug: "register-to-vote-armed-forces",
                                 created_at: Time.zone.now - 1.day, assigned_to_id: user.id)
    # This older edition should be excluded from results
    FactoryBot.create(:transaction_edition,
                      title: "Old GOV.UK page", slug: "old-gov-uk-page",
                      created_at: Time.zone.now - 3.years, assigned_to_id: user.id)
