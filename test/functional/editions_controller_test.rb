...
class EditionsControllerTest < ActionController::TestCase
  ...
  context "#tagging_breadcrumb_page" do
    ...
    context "user does not have editor permissions" do
      ...
      should "render an error message if the user is not a govuk_editor and try to remove breadcrumb" do
        user = FactoryBot.create(:user)
        login_as(user)
      ...
      should "render an error message if the user has welsh_editor permission and the edition is not Welsh and try to remove breadcrumb" do
        login_as_welsh_editor
