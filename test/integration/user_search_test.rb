...
class UserSearchTest < LegacyIntegrationTest
  ...
  test "selecting another user" do
    guides_1 = FactoryBot.build(:guide_edition)
    guides_2 = FactoryBot.build(:guide_edition, title: "if elephants could fly")
    other_user = FactoryBot.create(:user, name: "Bob", uid: "bob")

    # Assigning manually so it doesn't show up in Alice's list too
    guides_1.assigned_to_id = other_user.id
    guides_1.save!
    ...
    assert page.has_content?(guides_1.title)
    assert page.has_no_content?(guides_2.title)
  end
