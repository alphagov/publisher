...
class HostContentUpdateEventTest < ActiveSupport::TestCase
  ...
  describe "#is_for_edition?" do
    ...
    describe "if the edition is archived" do
      ...
      it "returns true if the event occurred after the published_at date and before the superseded date" do
        event = FactoryBot.build(:host_content_update_event, created_at: rand(published_at..superseded_at))
      ...
      it "returns false if the event occurred after the superseded date" do
        event = FactoryBot.build(:host_content_update_event, created_at: superseded_at + 3.days)
