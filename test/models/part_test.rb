...
class PartTest < ActiveSupport::TestCase
  context "validations" do
    ...
    context "when the part is not persisted" do
      setup do
        edition = create(:guide_edition)
        @part = edition.parts.build
      end

      should "not include the chapter number in the title error message" do
        @part.title = nil
        @part.valid?
        assert_not_includes @part.errors[:title], "Enter a title for Chapter 2"
      end

      should "not include the chapter number in the slug presence error message" do
        @part.slug = nil
        @part.valid?
        assert_not_includes @part.errors[:slug], "Enter a slug for Chapter 2"
      end

      should "not include the chapter number in the slug format error message" do
        @part.slug = "***"
        @part.valid?
        assert_not_includes @part.errors[:slug], "Slug for Chapter 2 can only consist of lower case characters, numbers and hyphens"
      end

      should "not include the chapter number in the slug 'video' error message" do
        @part.slug = "video"
        @part.valid?
        assert_not_includes @part.errors[:slug], "Slug for Chapter 2 can not be 'video'"
      end
    end
  end
end
