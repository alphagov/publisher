module Common
  ...
  included do
    ...
    def series
      self.class.where(panopticon_id:)
    end
    ...
    def self.find_or_create_from_panopticon_data(panopticon_id, importing_user)
      # byebug
      existing_publication = content_type.where(panopticon_id:)
    ...
    def type_specific_field_keys
      (fields.keys - content_type.fields.keys).map(&:to_sym)
    end

    def common_type_specific_field_keys(target_class)
      ((fields.keys & target_class.fields.keys) - content_type.fields.keys).map(&:to_sym)
    end
