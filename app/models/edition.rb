...
class Edition < ApplicationRecord
  ...
  def self.delegated_types
    %w[AnswerEdition GuideEdition ProgrammeEdition TransactionEdition HelpPageEdition SimpleSmartAnswerEdition CompletedTransactionEdition LicenceEdition LocalTransactionEdition PlaceEdition LicenceEdition PopularLinksEdition]
  end
  ...
  scope :slug_title_and_overview_search,
        lambda { |term|
          regex = "%#{term}%"
          where("title ILIKE ? OR slug ILIKE ? OR overview ILIKE ?", regex, regex, regex)
        }
  ...
  scope :search_title_and_slug,
        lambda { |term|
          regex = "%#{term}%"
          where("title ILIKE ? OR slug ILIKE ?", regex, regex)
        }
  ...
  def self.for_user(user_id)
    editions = Action.where(requester_id: user_id).or(Action.where(recipient_id: user_id)).map {|action| action.edition}
  ...
  def indexable_content
    content = respond_to?(:parts) ? indexable_content_with_parts : indexable_content_without_parts
    if editionable.respond_to?(:indexable_content)
      content = "#{content} #{editionable.indexable_content}"
    end
    content.strip
  end
  ...
  def build_clone(target_class = nil)
    ...
    # if cloning_between_parted_types?(new_edition.editionable)
    #   Parted.build_clone(new_edition.editionable, self.editionable)
    # end
  ...
  def exact_route?
    self.editionable.class.name.in? EXACT_ROUTE_EDITION_CLASSES
  end
  ...
  def content_id
    self.editionable_type == "PopularLinksEdition" ? self.editionable.content_id : self.artefact.content_id
  end
