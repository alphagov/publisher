...
class FactCheckEmailHandler
  ...
  def authenticate(gmail)
    ...
    parsed_key = StringIO.new(ENV.fetch("FACT_CHECK_API_KEY"))
  ...
  # Three utility methods to assist in testing, as webmock for Gmail API is overly complicated when these will do for
  # CI. If these fail it is outside of our scope as these are direct API calls to Google.
