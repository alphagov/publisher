...
class FactCheckEmailHandler
  ...
  def process_message(message)
    ...
    subject = message.payload.headersfind {|h| h.name.downcase =='subject'}&.value
    body = # Need to extract this from message object
  ...
  def authenticate(google_mail)
    scopes = ['https://www.googleapis.com/auth/gmail.labels', 'https://www.googleapis.com/auth/gmail.metadata', 'https://www.googleapis.com/auth/gmail.modify']
    
    authorizer = Google::Auth::ServiceAccountCredentials.make_creds(ENV.fetch("FACT_CHECK_API_KEY"), scopes)

    google_mail.authorization = authorizer
    client = authorizer.dup
    client.sub = ENV.fetch("FACT_CHECK_ADDRESS")
    authorizer.fetch_access_codes!
  end
  ...
  def process
    ...
    google_mail = Google::Apis::GmailV1::GmailService.new
    authenticate(google_mail)

    message_list = google_mail.list_user_messages('me').messages
    message_list.each do |message|
      message_content = google_mail.get_user_message('me', message.id)
      process_message(message_content)

      # This delete has yet to be moved to google API system. Need to archive it.
      message.mark_for_delete = true
      yield(message) if block_given?
    rescue StandardError => e
      Rails.logger.debug "UnableToProcessError: Failed to process message '#{message.payload.headersfind {|h| h.name.downcase =='subject'}&.value}': #{e.message}"
