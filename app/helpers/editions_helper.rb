module EditionsHelper
  ...
  def edition_status_hint(edition)
    return unless %w[fact_check in_review scheduled_for_publishing].include? edition.state

    hint_text = case edition.state
                when "fact_check"
                  "#{time_ago_in_words(edition.updated_at)} ago"
                when "in_review"
    ...
    sanitize("<p class='govuk-hint govuk-!-margin-top-1 govuk-!-margin-bottom-0'>#{hint_text}</p>")
  end
