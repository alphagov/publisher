...
class BusinessSupportContentImporter
  
  CONTENT_FILENAME = 'unwritten-licences'
  ...
  def self.run(method, importing_user=User.first)
    importer = BusinessSupportContentImporter.new(importing_user)
    
    importer.csv_data(CONTENT_FILENAME).each do |row|
  ...
  def report row
    identifier = row['OID']
    existing_editions = BusinessSupportEdition.where(licence_identifier: identifier)
    ...
    if existing_editions.size > 0
      ...
      if api_response && api_response.code == 201 # 'created' http response code
        ...
        if edition
          ...
          puts "Created BusinessSupportEdition in publisher with panopticon_id: #{artefact_id}, licence_identifier: #{identifier}"       
          @imported << edition
        else
          @failed << identifier
          puts "Failed to import BusinessSupportEdition into publisher. Identifier: #{identifier}."
        end
      else
        @failed << identifier
        puts "Failed to import BusinessSupportEdition via panopticon API. Identifier: #{identifier}."
      end
    end
  end
  ...
  def formatted_result(import=true)
    puts "--------------------------------------------------------------------------"
    puts "#{existing.size} BusinessSupportEditions skipped."
    puts "#{imported.size} BusinessSupportEditions#{(import ? '' : ' can be')} imported."
    puts "--------------------------------------------------------------------------"
  end
